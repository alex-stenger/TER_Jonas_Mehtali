...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, norm_layer, mid_channels=None, return_features=None):
        super().__init__()
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.BatchNorm2d(mid_channels) if norm_layer=="bn" else nn.InstanceNorm2d(mid_channels)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(mid_channels, out_channels, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(out_channels) if norm_layer=="bn" else nn.InstanceNorm2d(out_channels)
        )
...
class Up(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, norm_layer, bilinear=True, skip_connection=True, return_features=None
    ):
        super().__init__()
        self.return_features = return_features
        ...
        if bilinear:
            ...
            self.conv = DoubleConv(in_channels, out_channels, in_channels // 2, norm_layer, return_features=return_features)
        else:
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)
            self.conv = DoubleConv(in_channels, out_channels, norm_layer, return_features=return_features)
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, norm_layer="bn", bilinear=False, return_features=None):
